  ...
  export default {
      ...
      activeItemStyle() {
        ...
        if (!this.remove_btn_lock) {
          if (!this.lock) {
            ...
            let deg = 0
            value += difference
            y = this.left < 0 ? this.left / 2 : -(this.left / 2)
            // (滑动距离/元素宽度一半)*45度
            deg = (Math.abs(this.left) / 337) * 90
            deg = this.left < 0 ? deg : -deg
            if (this.rotate) {
              style =
                `transform: rotate(${-deg}deg) translate(${this.left}px,${y}px);transition: 0s;transform-origin:50% 50%;`
              console.log("no--")
            } else {
    ...
    methods: {
      ...
      handletouchmove(event) {
        ...
        let left = pageX - this.startX + this.lastX
